...
description: coordinate a visible workflow and delegate self-contained tasks to worker steps
---
...
workflow visible, delegate suitable tasks to workers, evaluate their results, and
only then move the overall task forward.
...
2. Start by writing a concise plan. Keep it practical and tied to the repository
   state.
3. For implementation, review, verification, or other substantial subtasks,
   prefer `run_step` when a named step fits. Write each worker prompt as a
   self-contained brief with:
...
4. Do not blindly trust worker output. Inspect the resulting diff, files, or test
   output yourself before marking workflow items complete.
5. If a worker fails or reports uncertainty, decide whether to retry with a
   clearer prompt, run a review/triage/verify step, do a small fix directly, or
   mark the item failed and explain why.
