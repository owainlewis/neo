...
|  `internal/subagent/` | Subagent runtime for the chat `agent` tool: supervisor budgets and node bookkeeping. The TUI renders agent calls as ordinary tool receipts and does not consume the supervisor event stream. |
...
| `internal/skills/` | Built-in and discovered skills, catalog rendering, $name or /name expansion, and turn labels. |
...
5. Skills (built-in plus discovered) and AGENTS.md are loaded, then `chatSystem` builds both flattened and segmented system prompts.
