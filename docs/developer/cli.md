...
- `/model` loads the active provider’s model catalogue asynchronously on first opening, with a spinner. Results are cached for the TUI session, including the OpenRouter default fallback if its five-second fetch fails. Fetch warnings appear in the picker. Closing the picker leaves the fetch running so reopening reuses it. Reopening also reuses any pending spinner tick; if a tick arrives while the picker is closed, animation stops until the picker opens again.
