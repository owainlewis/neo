...
- `src/content/docs/docs/reference/**`: **generated, not committed.** `scripts/prepare-docs.mjs`
  ...
  into site routes. Don't hand-edit files here. They are wiped and regenerated on every run. To
...
regardless of source order. You still need one block per `data-theme` value (`light`, `dark`)
